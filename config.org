...
;;let the output from the shell dealed by the terminal
;;emulator itself.
(defun external-alacritty-shell-command (COMMAND)
  "Run a terminal command in an external xterm window."
  (interactive "sShell Command: ")
  (shell-command (concat "alacritty -e " COMMAND)))


#+end_src
** key-bind
:LOGBOOK:
CLOCK: [2023-03-05 Sun 14:35]--[2023-03-05 Sun 15:00] =>  0:25
:END:
...
(map! (:leader
      ...
      (:desc "look at the /dev/ttyACM0 in alacritty"
       "d e v A" (lambda () (interactive)(external-alacritty-shell-command "~/bin/arduino-cli monitor -p /dev/ttyACM0")))))


#+end_src
#+RESULTS:
| lambda | nil | (interactive) | (external-alacritty-shell-command ~/bin/arduino-cli monitor -p /dev/ttyACM0) |
